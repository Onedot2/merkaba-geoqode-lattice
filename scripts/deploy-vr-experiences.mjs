...
const filterCategory =
  (args.find((a) => a.startsWith("--category=")) || "").replace(
    "--category=",
    "",
  ) || null;
const filterStatus =
  (args.find((a) => a.startsWith("--status=")) || "").replace(
    "--status=",
    "",
  ) || "live";
...
function log(msg) {
  console.log(`[deploy-vr] ${msg}`);
}
function warn(msg) {
  console.warn(`[deploy-vr] ⚠️  ${msg}`);
}
function ok(msg) {
  console.log(`[deploy-vr] ✅ ${msg}`);
}
function fail(msg) {
  console.error(`[deploy-vr] ❌ ${msg}`);
}
...
async function main() {
  ...
  log(
    `Loaded taxonomy v${version} — ${platform} — ${categories.length} categories`,
  );
  log(
    `CANONICAL_ARCHITECTURE: ${CANONICAL_ARCHITECTURE} · PHI=${PHI} · PSI=${PSI}`,
  );
  ...
  log(
    `Experiences to publish: ${toPublish.length} | Skipping: ${toSkip.length}`,
  );
  console.log("");

  // ── Print plan table ───────────────────────────────────────────────────────
  console.log(
    pad("ID", 24) + pad("Category", 14) + pad("Status", 10) + "Freq/Lattice",
  );
  ...
  for (const { xp, cat } of toPublish) {
    console.log(
      pad(xp.id, 24) +
        pad(cat.display, 14) +
        pad(xp.status, 10) +
        `${xp.frequencyHz}Hz · N${xp.latticeNode}`,
    );
  }
  console.log("");
  ...
  for (const { xp, cat } of toPublish) {
    ...
    try {
      ...
      if (isOk) {
        ...
        fail(
          `Failed: ${label} — HTTP ${status}: ${JSON.stringify(json).slice(0, 120)}`,
        );
        results.failed++;
        results.details.push({
          id: xp.id,
