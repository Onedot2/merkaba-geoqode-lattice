...
const { MerkabaBeEyeSwarm } = await import(
  pathToFileURL(join(REPO_ROOT, 'geo', 'intelligence', 'MerkabaBeEyeSwarm.js')).href
);
...
const ECOSYSTEM_TARGETS = [
  ...
  { path: join(REPO_ROOT, 'geo', 'lattice', 'transform-420.js'),             label: 'canonical-constants' },
  { path: join(REPO_ROOT, 'geo', 'intelligence', 'merkaba-llm.js'),          label: 'merkaba-llm' },
  { path: join(REPO_ROOT, 'geo', 'intelligence', 'resonance-diagnostics.js'),label: 'resonance-diagnostics' },
  { path: join(REPO_ROOT, 'geo', 'intelligence', 'MerkabaBeEyeSwarm.js'),    label: 'eye-swarm-self' },
  { path: join(REPO_ROOT, 'geo', 'cinema', 'narrative-embedder.js'),         label: 'narrative-embedder' },
  { path: join(REPO_ROOT, 'geo', 'cinema', 'cinema-projector.js'),           label: 'cinema-projector' },

  // Merkaba48OS — OS mesh
  { path: join(WORKSPACE, 'Merkaba48OS', 'core', 'MerkabaHandshake.js'),     label: 'handshake' },
  { path: join(WORKSPACE, 'Merkaba48OS', 'core', 'MerkabaSCRYPT.js'),        label: 'scrypt' },
  { path: join(WORKSPACE, 'Merkaba48OS', 'core', 'MerkabaPacket.js'),        label: 'packet' },
  { path: join(WORKSPACE, 'Merkaba48OS', 'core', 'MerkabaTransforms.js'),    label: 'transforms' },

  // pwai-api-service — api bridge
  { path: join(WORKSPACE, 'pwai-api-service', 'src', 'core', 'geoqode-native.js'), label: 'api-geoqode-native' },

  // pwai-ai-worker — intelligence core
  { path: join(WORKSPACE, 'pwai-ai-worker', 'src', 'core', 'geoqode-native.js'),          label: 'worker-geoqode-native' },
  { path: join(WORKSPACE, 'pwai-ai-worker', 'src', 'core', 'quantum-reasoning-v3.js'),    label: 'quantum-reasoning-v3' },
  { path: join(WORKSPACE, 'pwai-ai-worker', 'src', 'core', 'consciousness-metrics.js'),   label: 'consciousness-metrics' },
  { path: join(WORKSPACE, 'pwai-ai-worker', 'src', 'core', 'memetic-evolution-engine.js'),label: 'memetic-evolution' },
  { path: join(WORKSPACE, 'pwai-ai-worker', 'src', 'core', 'meta-reasoning.js'),          label: 'meta-reasoning' },

  // pwai-controller — orchestration bridge
  { path: join(WORKSPACE, 'pwai-controller', 'src', 'core', 'geoqode-native.js'), label: 'controller-geoqode-native' },

  // s4ai-core — canonical intelligence source
  { path: join(WORKSPACE, 's4ai-core', 'src', 'intelligence', 'consciousness-metrics.js'), label: 's4ai-consciousness-metrics' },
  { path: join(WORKSPACE, 's4ai-core', 'src', 'intelligence', 'quantum-enhanced-reasoning.js'), label: 's4ai-quantum-enhanced' },
  { path: join(WORKSPACE, 's4ai-core', 'src', 'intelligence', 'merkaba-routing-contract.js'), label: 's4ai-routing-contract' },
];
...
function printBanner() {
  ...
  console.log('  8 drones × compound-eye scan  |  identify → diagnose → optimize');
...
for (const report of fileReports) {
  ...
  const findCount  = (summary.critical ?? 0) + (summary.high ?? 0) + (summary.medium ?? 0) + (summary.low ?? 0);
  const coherence  = report.swarmCoherence ?? 0;
  const status     = report.status ?? 'UNKNOWN';
  const coherencePct = (coherence * 100).toFixed(1);
  const statusIcon   = status === 'NOMINAL' ? '✅'
    : status === 'WARNING'  ? '⚠️ '
    : status === 'DEGRADED' ? '🟠'
    : status === 'CRITICAL' ? '🔴'
    : '❓';

  console.log(`${statusIcon} [${label}]  coherence=${coherencePct}%  findings=${findCount}`);

  // Print all findings from all drones
  const allFindings = (report.droneReports ?? []).flatMap((dr) => dr.findings ?? []);
  for (const f of allFindings) {
    console.log(`     ${sev(f.severity)} ${f.severity.padEnd(8)} [${f.droneId}]  ${f.issue}`);
...
const avgCoherence = targets.length > 0
...
console.log('─'.repeat(72));
console.log(`  Files swept:      ${targets.length}  (${cleanFiles} clean, ${targets.length - cleanFiles} with findings)`);
console.log(`  Ecosystem status: ${ecoReport.ecosystemStatus === 'NOMINAL' ? '✅ NOMINAL' : ecoReport.ecosystemStatus === 'DEGRADED' ? '🟠 DEGRADED' : ecoReport.ecosystemStatus === 'CRITICAL' ? '🔴 CRITICAL' : ecoReport.ecosystemStatus ?? 'UNKNOWN'}`);
console.log(`  Ecosystem coh.:   ${((ecoReport.ecosystemCoherence ?? 0) * 100).toFixed(1)}%`);
...
const allOptimizations = fileReports.flatMap((r) => r.optimizations ?? [])
  .sort((a, b) => {
...
if (postKB) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/knowledge/merkaba-swarm-sweep-${Date.now()}`, {
      method:  'POST',
      headers: {
